...
<head>
  ...
  <script language="javascript">
    ...
    { 
      ...
      docprint.document.write('<img src="rlcs.png" style="width:1000px;height:70px"'); 
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                ...
                <form method="post" action="">
                    ...
                    <label><input name="dyear" id="dyear" type="number" class="form-control input-sm" placeholder="" aria-controls="dataTables-example"></label>
                   <!-- <label><input type="button" value="Search" class="form-control" placeholder="" aria-controls="dataTables-example"></label> -->
                   <label><button class="form-control" type="submitdate" name="submitdate" >Submit</button></label>
                    </div>
                    </form>
                <div class="content" id="content"  style="text-align: right">
                    ...
                    if(isset($_POST["submitdate"]))
                    {
                      $dmonth=$_POST["dmonth"];
                      $dyear=$_POST["dyear"];
                      echo'<center><p style="font-size:48px;">';
                      echo 'Sales Report: '.$dmonth.' '.$dyear;
                      echo'</p></center>';
                        include('connect.php'); 
                        if(!empty($dmonth) && !empty($dyear)){
                          $sql = "SELECT *, category as cat, sum(qty) as qcount FROM sales_order where  omonth = '$dmonth' && oyear = '$dyear' GROUP BY category";
                        } else if (!empty($dmonth)){
                          $sql = "SELECT *, category as cat, sum(qty) as qcount FROM sales_order where  omonth = '$dmonth' GROUP BY category";
                        } else if (!empty($dyear)){
                          $sql = "SELECT *, category as cat, sum(qty) as qcount FROM sales_order where  oyear = '$dyear' GROUP BY category";
                        } else if (empty($dmonth) && empty($dyear)){
                          $sql = "SELECT *, category as cat, sum(qty) as qcount FROM sales_order GROUP BY category";
                        } else {
                          $sql = "SELECT *, category as cat, sum(qty) as qcount FROM sales_order GROUP BY category";
                        }
                    ...
                    <div class="row">
                        <?php 
                        // include('connect.php'); 
                        // $sql = "SELECT *, category as cat, count(category) as qcount FROM sales_order where omonth = 'Augusto' && oyear = 2018 GROUP BY category";
                        // $query = $db->prepare($sql); 
                        // $query->execute();
                        // $fetch = $query->fetchAll();
                        // foreach ($fetch as $key => $value) {
                   ...
                   <table border="1" cellpadding="5" width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                    ...
                    <tbody>
                      ...
                      if(!empty($dmonth) && !empty($dyear)){
                        $result = $db->prepare("SELECT * FROM sales_order where omonth = '$dmonth' && oyear = '$dyear'");
                      } else if (!empty($dmonth)){
                        $result = $db->prepare("SELECT * FROM sales_order where omonth = '$dmonth'");
                      } else if (!empty($dyear)){
                        $result = $db->prepare("SELECT * FROM sales_order where oyear = '$dyear'");
                      } else if (empty($dmonth) && empty($dyear)){
                        $result = $db->prepare("SELECT * FROM sales_order");
                      } else {
                        $result = $db->prepare("SELECT * FROM sales_order");
